...
var OrgChart = class {
  ...
  render() {
    ...
    if (!attrs.data || attrs.data.length == 0) {
      ...
      if (attrs.container) {
        select_default(attrs.container).select(".nodes-wrapper").remove();
        select_default(attrs.container).select(".links-wrapper").remove();
        select_default(attrs.container).select(".connections-wrapper").remove();
      }
      return this;
    }
  ...
  update({ x0, y0, x = 0, y = 0, width, height }) {
    ...
    nodeUpdate.select(".node-button-text").attr("text-anchor", "middle").attr("alignment-baseline", "middle").attr("font-size", ({ children }) => {
      if (children)
        return 40;
      return 26;
    }).text(({ children }) => {
    ...
    nodeExitTransition.attr("opacity", 1).transition().duration(attrs.duration).attr("transform", (d) => {
      let { x: x2, y: y2, width: width2, height: height2 } = maxDepthNode.parent || {};
      const ex = attrs.layoutBindings[attrs.layout].nodeJoinX({ x: x2, y: y2, width: width2, height: height2 });
      const ey = attrs.layoutBindings[attrs.layout].nodeJoinY({ x: x2, y: y2, width: width2, height: height2 });
